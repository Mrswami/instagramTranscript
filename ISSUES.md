...
| **ISSUE-07** | Link Parsing | Shared links (`/share/p/...`, `?igsh=...`) extracted invalid shortcodes (`"p"`). | Regex matched `/share/` as the video ID rather than the true shortcode token. | Overhauled `extract_shortcode()` and `validate_url()` to support all share URLs and query strings. | ✅ Fixed |
| **ISSUE-08** | Downloader | `yt-dlp` failed when Instagram rate-limited cloud IPs or updated page structure. | Instagram scraper IP blocks and single-point-of-failure extraction. | Built 4-tier fallback system: `yt-dlp` -> DDInstagram OpenGraph scraper -> Instagram Embed scraper -> Public API. | ✅ Fixed |
| **ISSUE-09** | Performance | Heavy local PyTorch Whisper CPU load caused high latency and cloud OOM crashes. | Running PyTorch Whisper model locally on CPU required 500MB+ RAM and 20s+ inference time. | Integrated OpenAI `whisper-1` Cloud API (`OPENAI_API_KEY`) for 2-second cloud transcription with automatic local fallback. | ✅ Fixed |
| **ISSUE-10** | Workaround | Private Reels, deleted posts, or rate-limited links blocked users completely. | External link scraping impossible for auth-gated or rate-limited posts. | Added Direct Video/Audio File Upload Drag & Drop in both `public/index.html` and Streamlit `app.py` UI. | ✅ Fixed |
